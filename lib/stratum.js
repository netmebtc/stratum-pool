...
var StratumClient = function(options){
    ...
    function sendJson(){
        if(!_this.socket.writable) return;
        var response = '';
    ...
    function setupSocket(){
        ...
        socket.on('error', function(err){
            if (err.code !== 'ECONNRESET')
                _this.emit('socketError', _this.getLabel() + ': ' + JSON.stringify(err));
        });
    }
    ...
    this.sendMiningJob = function(jobParams){
        ...
        if (lastActivityAgo > options.connectionTimeout * 1000){
            _this.emit('socketTimeout',  _this.getLabel() + ': '+'last submitted a share was ' + (lastActivityAgo / 1000 | 0) + ' seconds ago');
            // _this.socket.destroy(); 
            return;
        }
...
//            _this.emit('client.disconnected', subscriptionId);            
        }).on('socketTimeout', function(){
            _this.removeStratumClientBySubId(subscriptionId);
//            _this.emit('client.disconnected', subscriptionId);
